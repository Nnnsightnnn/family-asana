...
.\scripts\admin.ps1 tail-logs 200       # last N lines of err.log + out.log (default 50)
...
Companion script `scripts/verify-backup.ps1` runs `PRAGMA integrity_check` against the most recent backup — schedule it weekly per [`WINDOWS-SETUP.md`](WINDOWS-SETUP.md).
...
If `sc query` shows the service stopped and it won't restart, the usual culprit is the `.env` file (typo) or the SQLite file (locked). Restore the latest backup from OneDrive if the DB is the problem.
...
- Where the SQLite backups land (OneDrive folder or B2 bucket)
